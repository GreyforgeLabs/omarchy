...
document.addEventListener('__omarchy_request', (event) => {
  ...
  if (request.kind === 'can-set') {
    ...
    const fields = ['id', 'kind', 'name', 'colors', 'backgroundUrl', 'backgroundUrls',
      'mode', 'iconsTheme', 'previewUrl', 'previewUnlockUrl', 'unlockUrl'];
    if (Object.keys(request).some((key) => !fields.includes(key))) {
      respond({ ok: false, error: 'unsupported theme specification' });
      return;
    }
    outbound = {
      ...
      mode: request.mode,
      iconsTheme: request.iconsTheme,
      previewUrl: request.previewUrl,
      previewUnlockUrl: request.previewUnlockUrl,
      unlockUrl: request.unlockUrl,
    };
  } else {
