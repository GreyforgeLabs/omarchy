...
function imageUrl(url) {
  if (typeof url !== 'string' || !url.length || url.length > 4096) {
    throw new Error('image url must be a nonempty string of at most 4096 characters');
  }
  let target;
  try {
    target = new URL(url);
...
async function fetchImage(url, { signal, maxBytes = MAX_IMAGE_BYTES, pngOnly = false } = {}) {
  const target = imageUrl(url);
  if (signal?.aborted) throw new Error('background download timed out');
  if (maxBytes <= 0) throw new Error('theme images exceed 8MB in total');
  const controller = new AbortController();
  const abort = () => controller.abort();
  signal?.addEventListener('abort', abort, { once: true });
  const timer = signal ? null : setTimeout(abort, DOWNLOAD_TIMEOUT_MS);
  const sizeError = maxBytes === MAX_IMAGE_BYTES
    ? 'background is larger than 8MB' : 'theme images exceed 8MB in total';
  let reader;
  try {
    ...
    if (pngOnly && type !== 'image/png') throw new Error('preview and unlock assets must be PNG');
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(type)) throw new Error('background is not a JPEG, PNG or WebP');
    ...
    while (true) {
      ...
      if (done) break;
      if (size + value.byteLength > maxBytes) throw new Error(sizeError);
      buffer.set(value, size);
      size += value.byteLength;
    }
    if (!size) throw new Error('background is empty');
    return { data: base64(buffer.subarray(0, size)), bytes: size };
  } catch (error) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  const { name, colors, backgroundUrl, backgroundUrls, mode, iconsTheme,
    previewUrl, previewUnlockUrl, unlockUrl } = message;
  const fields = ['type', 'name', 'colors', 'backgroundUrl', 'backgroundUrls', 'mode',
    'iconsTheme', 'previewUrl', 'previewUnlockUrl', 'unlockUrl'];
  const urls = backgroundUrls === undefined ? (backgroundUrl ? [backgroundUrl] : []) : backgroundUrls;
  if (typeof name !== 'string' || !name.length || name.length > MAX_THEME_NAME_CHARS || /[\x00-\x1f\x7f]/.test(name) ||
      !colors || typeof colors !== 'object' || Array.isArray(colors) || Object.hasOwn(colors, 'mode') || Object.keys(colors).length > MAX_COLORS ||
      ...
      (backgroundUrl !== undefined && (typeof backgroundUrl !== 'string' || backgroundUrl.length > 4096 || backgroundUrls !== undefined)) ||
      !Array.isArray(urls) || urls.length > MAX_BACKGROUNDS ||
      !urls.every((url) => typeof url === 'string' && url.length > 0 && url.length <= 4096) ||
      (mode !== undefined && mode !== 'dark' && mode !== 'light') ||
      (iconsTheme !== undefined && (typeof iconsTheme !== 'string' || !iconsTheme.length || iconsTheme.length > 64 ||
        !/^[A-Za-z0-9]/.test(iconsTheme) || /[^A-Za-z0-9._+-]/.test(iconsTheme))) ||
      [previewUrl, previewUnlockUrl, unlockUrl].some((url) => url !== undefined &&
        (typeof url !== 'string' || !url.length || url.length > 4096)) ||
      (previewUnlockUrl === undefined) !== (unlockUrl === undefined)) {
  ...
  const assets = urls.map((url) => ({ field: 'backgrounds', url, pngOnly: false }));
  for (const [field, url] of [['preview', previewUrl], ['previewUnlock', previewUnlockUrl], ['unlock', unlockUrl]]) {
    if (url !== undefined) assets.push({ field, url, pngOnly: true });
  }
  try {
    // Validate the entire list before issuing even the first network request.
    for (const asset of assets) imageUrl(asset.url);
  } catch (error) {
  ...
  (async () => {
    try {
      const payload = { type: 'install-theme', name, colors, backgrounds: [] };
      if (mode !== undefined) payload.mode = mode;
      if (iconsTheme !== undefined) payload.iconsTheme = iconsTheme;
      if (assets.length) {
        const controller = new AbortController();
        // One deadline and byte budget cover the entire sequential batch.
        const timer = setTimeout(() => controller.abort(), DOWNLOAD_TIMEOUT_MS);
        let remaining = MAX_IMAGE_BYTES;
        try {
          for (const asset of assets) {
            const image = await fetchImage(asset.url, { signal: controller.signal, maxBytes: remaining, pngOnly: asset.pngOnly });
            remaining -= image.bytes;
            if (asset.field === 'backgrounds') payload.backgrounds.push(image.data);
            else payload[asset.field] = image.data;
          }
        } finally {
          clearTimeout(timer);
          controller.abort();
